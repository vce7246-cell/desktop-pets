...
- [ ] In `pet_window.py`, subclass `QWidget`.
...
- [ ] `QLabel` is added to `PetWindow` via a `QVBoxLayout` (stretch + center).
...
- [ ] Implement the transition function `update(cursor_speed, distance_to_pet, mouse_still_duration)` that returns the new state:
  - `FOLLOWING`: when mouse is moving (`cursor_speed > 0`).
  - `RUNNING`: when `cursor_speed > 600` px/s.
  - `IDLE`: when `mouse_still_duration > 2.0` seconds.
  - `EXCITED`: when `distance_to_pet < 150` pixels AND `cursor_speed > 0`.
  - When no condition matches (e.g., speed=0 but still_duration < 2.0): **keep previous state**.
- [ ] State priority (highest wins): **RUNNING > EXCITED > IDLE > FOLLOWING**.
- [ ] State machine is **pure Python** — no QObject inheritance. State changes are detected by comparing `current_state` before/after `update()` in the game loop.
...
| (0, 500, 3.0) | IDLE |
| (200, 80, 0.5) | EXCITED |
| (800, 80, 0.0) | RUNNING | RUNNING priority > EXCITED |
| (0, 80, 0.5) | (previous) | Speed=0, still <2s — keep prev state |
| (0, 80, 3.0) | IDLE | Still >2s; EXCITED requires speed>0 |
